...
inline static void allocateOnDemand ( IplImage **img, CvSize size, int depth, int channels )
	...
	*img = cvCreateImage( size, depth, channels );
	...
	struct Pointer FramesToPointerBM( struct TwoFrames tf, CvSize fs ){
				...
				cvConvertImage(tf.frame1, frame1_1C, CV_CVTIMG_FLIP);	//Übertragen au das Bild des erste Frames
...
//	ptr_vv.p1.x=ptr_vv.p1.y=ptr_vv.p2.x=ptr_vv.p2.y=0;
	...
	fprintf(datei,"Loop;Zeit[ms];Länge;LängeX;LängeY;LK_winSize\n");
	...
	while(c != 'q'){
		...
		//WinSize für CvCalcOpticalFlowLK 
